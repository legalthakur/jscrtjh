...
function insertNewCard(data){
  ...
  <div class="card-body">
    <h5 class="card-title">${data.sevierty}</h5><span id="badge" class="badge text-bg-success">Open</span>
    <p class="card-text">${data.description}</p>
    <p>Assigned to: ${data.assignedTo}</p>
    <button  class="btn btn-warning prev btn-sm">Prev</button>
    <button  class="btn btn-danger del btn-sm">Delete</button>
    <button  class="btn btn-success next btn-sm">Next</button>
    <button style="margin: 5px;"  class="btn btn-success btn-sm close">Close</button>
  </div>
</div>`;
...
closeButtons.forEach(button=>{
  button.addEventListener('click',()=>{
    let bdge=document.getElementById("badge");
    bdge.innerHTML="Close";
